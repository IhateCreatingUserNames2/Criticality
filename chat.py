...
class MemoryBlossomChatbot:
    def __init__(
        ...
        :param openai_api_key: OpenAI API key (uses env var if not provided)
        ...
        # Load previous memories if persistence is enabled
    ...
    def chat(self, user_message: str) -> str:
        ...
        # 3) Call OpenAI
        # 3) Call OpenAI and assess Criticality
        try:
            # First generation
            ...
            while criticality_zone != 1 and attempt < max_regeneration_attempts:
                ...
                if criticality_zone == 0:
                    self.temperature = min(1.2, self.temperature + 0.2)  # Ordered: increase creativity
                elif criticality_zone == 2:
                    self.temperature = max(0.2, self.temperature - 0.2)  # Chaotic: decrease creativity
        ...
        # 🧠 4) Assess Criticality of the AI Response
        criticality_zone = assess_criticality(ai_response, user_message, logprobs_data)
        ...
        # 5) Store reply as a memory & update history
        self.memory_blossom.add_memory(
            content=ai_response,
